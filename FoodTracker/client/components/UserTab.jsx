...
const RECOMMENDATION_FIELD_ORDER = Object.keys(EMPTY_RECOMMENDATIONS);
...
const UserTab = () => {
  ...
  const [editableRecommendations, setEditableRecommendations] = useState(EMPTY_RECOMMENDATIONS);
  ...
  const [showCustomRecommendationEditor, setShowCustomRecommendationEditor] = useState(false);
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setEditableRecommendations(EMPTY_RECOMMENDATIONS);
      } else {
  ...
  const formatRecommendationLabel = (key) => key
    .replace(/_/g, ' ')
    .replace(/\b(kcal|mg|mcg|g|DHA|EPA)\b/g, (match) => match)
    .replace(/\b\w/g, (char) => char.toUpperCase())
    .replace('Kcal', '(kcal)')
    .replace('Mg', '(mg)')
    .replace('Mcg', '(mcg)')
    .replace(' G', ' (g)')
    .replace(' Dha Epa', ' DHA EPA');

  const getRecommendationStep = (key) => key === 'Calories_kcal' ? '1' : '0.01';
  ...
  const updateWithFormula = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(data);
        const saveResponse = await fetch(apiUrl('/api/recommendations'), {
          method: 'DELETE',
        });

        if (!saveResponse.ok) {
          throw new Error('Failed to persist formula recommendations');
        }

        const savedRecommendations = await saveResponse.json();
        setRecommendations(savedRecommendations);
        setEditableRecommendations(savedRecommendations);
        setSaveMessage('Recommendations reset to calculated defaults successfully!');
        setTimeout(() => setSaveMessage(""), 3000);
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data) {
          ...
          if (!saveResponse.ok) {
            throw new Error('Failed to persist AI recommendations');
          }
          ...
          setSaveMessage(result.search?.enabled
            ? 'AI recommendations with web search saved successfully!'
            : 'AI recommendations saved successfully!');
          setTimeout(() => setSaveMessage(""), 3000);
        } else {
  ...
  const openRecommendationEditor = () => {
    ...
    setShowCustomRecommendationEditor(true);
  };

  const handleRecommendationInputChange = (key, value) => {
  ...
  const saveCustomRecommendations = async () => {
    setSavingRecommendations(true);

    try {
      const normalizedRecommendations = RECOMMENDATION_FIELD_ORDER.reduce((accumulator, key) => {
        const rawValue = editableRecommendations[key];
        const parsedValue = Number.parseFloat(rawValue);

        if (!Number.isFinite(parsedValue) || parsedValue < 0) {
          throw new Error(`Please enter a valid value for ${formatRecommendationLabel(key)}`);
        }
      ...
      const response = await fetch(apiUrl('/api/recommendations'), {
        ...
        body: JSON.stringify(normalizedRecommendations),
      });
      ...
      setEditableRecommendations(result);
      setShowCustomRecommendationEditor(false);
      setSaveMessage('Custom recommendations saved successfully!');
      setTimeout(() => setSaveMessage(''), 3000);
    } catch (error) {
      console.error('Error saving custom recommendations:', error);
      setSaveMessage(error.message || 'Failed to save recommendations');
    } finally {
      setSavingRecommendations(false);
    }
  };

  const resetCustomRecommendations = async () => {
    setSavingRecommendations(true);
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok) {
        throw new Error(result.error || 'Failed to reset recommendations');
      }
      ...
      setSaveMessage(error.message || 'Failed to reset recommendations');
    } finally {
      setSavingRecommendations(false);
    }
  };
  ...
  return (
    <div className="user-tab">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <div>
                ...
                <p style={{ color: '#ccc', lineHeight: '1.5', margin: 0 }}>
                  Your personalized daily nutrient targets based on your profile settings. You can also override them manually.
                </p>
              </div>
      ...
      {showCustomRecommendationEditor && (
        <div
          className="modal-overlay"
          style={{
            position: 'fixed',
          ...
          onClick={() => !savingRecommendations && setShowCustomRecommendationEditor(false)}
        >
          <div
            className="modal-content"
            style={{
          ...
          >
            ...
            <p style={{ color: '#ccc', marginBottom: '1.5rem', lineHeight: '1.6' }}>
              These values start from the calculated defaults, but you can set any target you prefer.
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
              {RECOMMENDATION_FIELD_ORDER.map((key) => (
                <div key={key} className="form-group">
                  <label htmlFor={`recommendation-${key}`}>{formatRecommendationLabel(key)}</label>
                  <input
                    id={`recommendation-${key}`}
                    type="number"
                    min="0"
                    step={getRecommendationStep(key)}
                    value={editableRecommendations[key] ?? ''}
                    onChange={(e) => handleRecommendationInputChange(key, e.target.value)}
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem' }}>
              <button
                onClick={saveCustomRecommendations}
                disabled={savingRecommendations}
                style={{
                  flex: 1,
                  padding: '0.85rem',
                  ...
                  border: 'none',
                  borderRadius: '6px',
                  cursor: savingRecommendations ? 'not-allowed' : 'pointer',
                  opacity: savingRecommendations ? 0.7 : 1,
                  fontWeight: '600'
                }}
              >
                {savingRecommendations ? 'Saving...' : 'Save Custom Targets'}
              </button>
              <button
                onClick={resetCustomRecommendations}
                disabled={savingRecommendations}
                style={{
                  flex: 1,
                  padding: '0.85rem',
            ...
            <button
              onClick={() => setShowCustomRecommendationEditor(false)}
              disabled={savingRecommendations}
              style={{
                marginTop: '0.75rem',
                width: '100%',
